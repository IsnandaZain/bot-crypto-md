...
class BotLogger:
    ...
    def print_position_summary(summary):
        ...
        if summary['total_positions'] == 0:
            ...
            for pos in summary['positions']:
                ...
                print(f"  Entry: ${pos['entry_price']:.2f} | Current Price: ${pos['current_price']:.2f}")
                print(f"  SL: ${pos['stop_loss']:.2f} | TP: ${pos['take_profit']:.2f}")
                print(f"  Qty: {pos['quantity']} | Margin: ${pos['margin_required']:.2f}")
                print(f"  {pnl_emoji} Unrealized PnL: ${pos['unrealized_pnl']:.2f} ({pos['unrealized_pnl_pct']:.2f}%)")
    ...
    def log_closed_positions(closed_positions):
        ...
        for pos in closed_positions:
            ...
            print(f"  {pnl_emoji} PnL: ${pos['pnl_usdt']:.2f} ({pos['pnl_pct']:.2f}%)")
    ...
    def print_new_signals(new_signals):
        ...
        if not new_signals:
            ...
            for r in new_signals:
                print(f"\n{r['symbol']} ({r['signal']})")
                print(f"  Entry: ${r['risk']['entry']:.2f}")
                print(f"  SL: ${r['risk']['stop_loss']:.2f} ({r['risk']['sl_pct_from_entry']})")
                print(f"  TP: ${r['risk']['take_profit']:.2f} ({r['risk']['tp_pct_from_entry']})")
                print(f"  Qty: {r['position']['quantity']}")
                print(f"  Margin: ${r['position']['margin_required']:.2f}")
